...
try {
    ...
    $auth = (new RequireOwner(new SessionManager($config), new UserRepository($db)))->authorize();
    ...
    Response::success($service->profile($owner));
} catch (ApiException $error) {
    Response::error($error->apiCode(), $error->getMessage(), $error->status(), $error->fields());
} catch (Throwable $error) {
    Response::error('SERVER_ERROR', 'Unable to load admin profile.', 500);
}
