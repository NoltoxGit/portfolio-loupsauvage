...
try {
    ...
    Response::success($service->syncContentBundles(queryId($request->query(), 'contentId'), $request->json()));
} catch (ApiException $error) {
    Response::error($error->apiCode(), $error->getMessage(), $error->status(), $error->fields());
} catch (Throwable $error) {
    Response::error('SERVER_ERROR', 'Unable to update creation bundles.', 500);
}

/**
 * @param array<string, mixed> $query
 */
function queryId(array $query, string $key): int
{
    $id = $query[$key] ?? null;

    if (!is_numeric($id) || (int) $id <= 0) {
        throw new ApiException('VALIDATION_ERROR', 'Invalid id.', 422, [
            $key => 'A valid numeric id is required.',
        ]);
    }
