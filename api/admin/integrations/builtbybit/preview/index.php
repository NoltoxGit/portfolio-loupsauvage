...
use LoupSauvage\Services\BuiltByBitPreviewService;
...
try {
    ...
    Response::success((new BuiltByBitPreviewService($config))->preview($request->json()));
} catch (ApiException $error) {
    Response::error($error->apiCode(), $error->getMessage(), $error->status(), $error->fields());
} catch (Throwable $error) {
    Response::error('SERVER_ERROR', 'Unable to preview BuiltByBit resource.', 500);
}
