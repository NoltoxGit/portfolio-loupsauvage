      ...
      <section class="admin-login-panel" data-admin-login-panel aria-labelledby="admin-login-title">
        <div class="admin-login-card">
          ...
          <details class="admin-reset-box">
            <summary>Reset le mot de passe</summary>
...
async function initAdmin() {
  ...
  try {
    ...
    renderAdminLogin("Impossible de joindre l'API admin. Verifie api/config.php et MariaDB.");
  }
}
...
document.addEventListener("click", async (event) => {
  if (event.target.closest("[data-admin-logout]")) {
    try {
      ...
      console.warn("Deconnexion admin incomplete.", error);
    }

    renderAdminLogin("Tu es deconnecte.");
    return;
  }
  ...
  if (event.target.closest("[data-admin-reset]")) {
    ...
    if (adminRemoteMode) {
      ...
      await saveAdminData("Donnees reinitialisees.");
      editingIndex = null;
      renderAdmin();
      return;
    }
...
document.addEventListener("submit", async (event) => {
  ...
  if (loginForm) {
    ...
    try {
      ...
      console.error("Connexion admin refusee.", error);
  ...
  if (resetPasswordForm) {
    ...
    try {
      ...
      showAuthMessage("Mot de passe mis a jour. Tu peux te connecter.");
    } catch (error) {
      console.error("Reset mot de passe refuse.", error);
      showAuthMessage("Reset impossible. Verifie le token et le nouveau mot de passe.", true);
    }
    return;
  }
  ...
  if (config.type === "buttons") {
    ...
    await saveAdminData("Boutons sauvegardes.");
    renderAdmin();
    return;
  }
